...
| 47 | `47-inject_tracr_buffer.md` | Adds the `__tracr_buffer` Out parameter to every kernel issuing `pld.system.notify` / `pld.system.wait`, giving codegen a GM region for AICore TraCR records. Runs after AutoDeriveTaskDependencies on purpose (a shared trace buffer must not become a task dependency edge) and immediately before MaterializeDistTensorCtx, which appends CommCtx params as a trailing suffix |
| 48 | `48-materialize_dist_tensor_ctx.md` | 47th pass (materializes explicit CommCtx params/args for DistributedTensor params; runs before the final Simplify) |
