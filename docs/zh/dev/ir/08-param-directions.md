...
| [Wrapper 恢复与调用点](#3-wrapper-恢复与调用点pass-41) | 41 | 每个 wrapper 的**有效**签名是什么？每个调用点实参的方向是什么？ |
...
`DeriveCallDirections`（pass 41）之后运行，而 `InitMemRef`（pass 34）作废了
