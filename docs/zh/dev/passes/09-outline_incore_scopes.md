...
对于生成的 InCore 函数，如果每个返回值都是 tensor，且都能唯一解析到不同的已捕获
`Out`/`InOut` 参数，外提器会按参数顺序同步排列返回值、返回类型和调用方投影。
`IfStmt` 结果只有在所有分支解析到同一参数时才算成功；存在新分配、scalar、歧义、
无法追踪或重复映射的输出时，整组契约保留原顺序，避免把 SSA 顺序猜成 `Out` 回写 ABI。
