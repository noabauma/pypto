...
| 类别 | 示例 | 行为 |
| ---- | ---- | ---- |
| **结构性** | TypeChecked, BreakContinueValid, NoRedundantBlocks, UseAfterDef, OutParamNotShadowed, NoNestedInCore, InOutUseValid, PipelineLoopValid, ArrayNotEscaped, ManualDepsOnSubmitOnly, AtomicAddDtypeValid, NoScalarKernelReturn | 始终为真。由 `VerificationInstrument` 在每个 Pass 执行前后验证；与 `GetVerifiedProperties()` 共有的子集还会在流水线启动时验证。不在 PassProperties 中声明。 |
| **流水线** | SSAForm, NoNestedCalls, HasMemRefs, ... | 由 Pass 产生/失效。按 Pass 声明的契约验证。 |

`GetStructuralProperties()` 返回 `{TypeChecked, BreakContinueValid, NoRedundantBlocks, UseAfterDef, OutParamNotShadowed, NoNestedInCore, InOutUseValid, PipelineLoopValid, ArrayNotEscaped, ManualDepsOnSubmitOnly, AtomicAddDtypeValid, NoScalarKernelReturn}`。这些由 `VerificationInstrument` **在每个 Pass 执行前后验证**。在**流水线启动时**，`PassPipeline::Run()` 仅额外验证与 `GetVerifiedProperties()` 共有的轻量子集（`GetStructuralProperties().Intersection(GetVerifiedProperties())`）——因此例如 `ArrayNotEscaped` 会在每个 Pass 前后验证，但不会在流水线启动时验证。由于没有 Pass 在 `required`/`produced`/`invalidated` 中声明它们，`VerificationInstrument` 将它们与 Pass 声明的属性合并，确保没有 Pass 破坏这些基本不变量。
...
| **AccToGmStoreValid** | AccToGmStoreValid | 每个源 tile 位于 **Acc** 的 `tile.store`，其目标 GM tensor 的 dtype 必须是 cube fix-pipe 能够收窄到的类型：Ascend910B 与 Ascend950 均为 `INT32/FP32/FP16/BF16`（`BackendHandler::SupportsAccToGmDtype`，与 pto-isa 非量化分支的 `CheckAcc2gm` 白名单以及 ptoas `pto.tstore` 的 "acc tstore dst element type" 规则一致；该钩子仍按后端区分，因为两个集合是各自 pin 目标的独立事实）。`INT8`/`INT16` 目标会被拒绝。该白名单是一组**目标类型**而非一组转换，因此通过它是必要而非充分条件：这次 store 还必须是 fix-pipe **能从当前累加器出发**完成的。非量化写回只做一种转换，`FP32 -> FP16` / `FP32 -> BF16`（`ir::CubeWritebackSupportsDataType`，与 pto-isa 的 `GetCastPreQuantMode` 一致，a2a3 与 a5 相同），所以 `INT32` 累加器只能以 `INT32` 离开。`INT8 x INT8` matmul 直接存入 `FP32` tensor 能通过白名单，但它依然是一次没有 scale 的反量化：ptoas 会接受它，随后 ccec 在 pto-isa 的 `TStoreAcc` 中报错（"the 2nd parameter maybe need a type `__cc__ float *`"）；若形状恰好能编译通过，kernel 会把原始累加器比特按 float 重新解释后写出。**由** `InferTileMemorySpace` **产生**并列入 `GetVerifiedProperties()`，因此 `PassPipeline` 在该 pass 之后立即自动验证。这个位置是关键的：合法性取决于 tile 的**内存空间**，而非用户可见的 dtype，因此无法更早判定——同一段 DSL 程序，当 matmul 结果经由 Vec（显式 `pl.cast` 在向量单元中收窄）时合法，留在 Acc 时则非法。缺少此检查时程序会一路走到 ptoas，由后者在 `pto.tstore` 校验阶段针对生成的 `.pto` 中某一行报错；而且这样一个 store 往往还会连带触发另一个看似无关的算子（int8 零初始化会下降为同样非法 dtype 的 `pto.texpands`），于是这个迟到的诊断只报出两个症状，从不指出根因。当后端未配置（无可校验的依据）或 tile 内存空间尚未解析时跳过。**修复方式**：先经向量单元收窄——用 `pl.cast` 将 matmul 结果转为目标 dtype 后再存储——或者累加到 `INT32`/`FP32` tensor 后再转换。 |
...
| **TileOps2D** | TileOps2D | InCore 函数中没有任何 tile 算子的结果或实参是 >2D tile，每个 `tile.transpose` 都是 codegen 就绪的 4 参数形式，每个 `tile.assemble` 的 offset 都是字面量 `(row, col)` 二元组。PTO 的 `tile_buf` 是 2D 的，而 PTO codegen 仅从 `shape_[0]` / `shape_[1]` 读取 tile 几何（`ExtractTileTypeInfo`），因此残留的 rank-3 tile 会以错误尺寸发射——`[2, 8, 128]` 渲染成 `rows=2, cols=8`，即 16 个元素而非 2048 个。`tile.load` 与 `tile.store` 属于例外：重写阶段给它们 2D tile，而其 tensor 侧的窗口实参合法地保留源的 ND 秩。`tile.reshape` 与 `tile.reinterpret_view` **不属于**例外——它们的秩来自字面量 shape 实参而非某个实参的类型，这正是该 pass 必须重写那个实参的原因，也正是 >2D 结果曾在此处不被察觉地存活下来的原因。**由** `FlattenTileNdTo2D` **产生**并列入 `GetVerifiedProperties()`，故 `PassPipeline` 在该 Pass 之后立即自动验证。**修复方式**：此处报错属于 `FlattenTileNdTo2D` 的编译器缺陷，而非用户编写错误。 |
...
| **NoScalarKernelReturn** | NoScalarKernelReturn | 任何 device 函数（InCore / AIC / AIV / Group / Spmd）都不得声明标量返回值——检查会递归进入 `TupleType`，因为 `-> pl.Tuple[...]` 标注只占 `return_types_` 的**一个**条目，而元组元素同样没有载体。这些函数类型的含义就是"一次可派发的任务"，而运行时的两条任务参数通道是分离的——`Arg::add_scalar` 只把标量按值传**入**，`TaskOutputTensors` 只返回 ChipTensor——因此这种返回值根本没有载体。Orchestration codegen 曾为它生成未定义标识符，后来又生成静默错误的 `= 0`（issue #631）。检查落在**声明**而非调用点：`Opaque` 父函数要到 `OutlineIncoreScopes` 才被提升为 `Orchestration`，按调用点检查会漏掉该 pass 之前写出的每一处派发。`Scalar[TASK_ID]` 同样被拒绝：调度句柄是追加在**调用**的 tuple 类型尾部、并由调用点从 `task_<n>_outs.task_id()` 绑定的，因此声明里带一个 TaskId 同样没有载体——而且会被误读：orchestration codegen 的 `IsSubmitCall` 判据正是「TupleType 的最后一个元素是 `Scalar[TASK_ID]`」，对这样的被调函数的一次普通调用会被当成任务派发来下降。核内的标量辅助函数应写成 `FunctionType::Inline`，由 `InlineFunctions` 在 codegen 之前展开。列入 **`GetStructuralProperties()`**，且不由任何 Pass 产生：这里不依赖任何下降结果，因此用户写出的非法签名在流水线入口即被拒绝，任何合成出该形态的 Pass 也会在自己的边界被抓住。`OutlineIncoreScopes` 通过把调用方可计算的标量外提出作用域体来维持该不变量（参见 [08-outline_incore_scopes.md](09-outline_incore_scopes.md)）。**修复方式**：把该值写入一个 `[1]` 的 tensor 输出，在派发之后用 `pl.tensor.read(t, [0])` 读回。 |
...
`DeriveCallDirections`（pass 41）之后运行——在那之前 wrapper 的签名读作 `In` 是合法的；
