...
**Pipeline 位置**：紧跟在 [`LegalizeTileCast`](17-legalize_tile_cast.md) 之后，先于 [`CanonicalizeTileSlice`](19-canonicalize_tile_slice.md) 与 [`InferTileMemorySpace`](20-infer_tile_memory_space.md)。此时 tile op 已是 2D，但 memory space 尚未推断。
...
[`ConvertTensorToTileOps`](11-convert_tensor_to_tile_ops.md) 会把所有 M 轴穿过的 2-D
...
- [`InferTileMemorySpace`](20-infer_tile_memory_space.md) —— 下游 pass；负责桥接本 pass 故意保留下来的 Vec/Acc 累加器
- [`LowerPipelineLoops`](31-lower_pipeline_loops.md) —— 消费本 pass 产生的 `ForKind::Pipeline` + `pipeline_stages=2`
