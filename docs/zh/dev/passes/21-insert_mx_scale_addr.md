...
[`InferTileMemorySpace`](20-infer_tile_memory_space.md) 解析完 `Left` / `LeftScale` / `Right` / `RightScale` 并插入必要的 `tile.move` 之后，本 pass 物化 A5 的 scale 地址绑定：
...
**流水线位置**：紧接在 [`InferTileMemorySpace`](20-infer_tile_memory_space.md) 之后，[`ResolveBackendOpLayouts`](22-resolve_backend_op_layouts.md) 之前。
...
- MX 操作数 memory space 求解：[`InferTileMemorySpace`](20-infer_tile_memory_space.md)
