...
**流水线位置**: 在 [`NormalizeReturnOrder`](28-normalize_return_order.md) 之后，紧接在 [`LowerPipelineToSlots`](30-lower_pipeline_to_slots.md) 之前，后者又紧接在 [`LowerPipelineLoops`](31-lower_pipeline_loops.md) 之前。跨核循环因此在这两个 pass 看到它之前就已被 skew（→ Sequential，且不带 `pipeline_stages` 标记），既不会被开槽也不会被复制。
...
- [`LowerPipelineLoops`](31-lower_pipeline_loops.md) —— 复制其余（同核）pipeline 循环以实现 ping-pong。
- [`CanonicalizeIOOrder`](32-canonicalize_io_order.md) —— 在 pipeline body 内聚类同核 IO（跨核循环到这里已是 Sequential，不再进入此 pass）。
- [`SplitVectorKernel`](26-split_vector_kernel.md) —— `UP_DOWN` vector 切分，与 skew 正交且可组合。
