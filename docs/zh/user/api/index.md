...
**从[算子目录](../ops/01-catalog.md)开始，而不是从这里。** 目录按族列出每个算子并各配一句话，且每个名字都链到上面这些页里对应的签名。本章回答「参数是什么」，目录回答「我要哪个算子」。
...
`mkdocs build --strict` 会因两类问题失败：docstring 的 `Args:` 写了签名里没有的参数；目录链接指向一个并未被渲染的符号。两者都是真实缺陷 —— 而且本章第一次构建时**两类都当场抓到了**。

## 参见

- [算子目录](../ops/01-catalog.md) —— 通向这些页面的分类索引。
- [选择命名空间](../ops/00-dispatch.md) —— `pl.` / `pl.tile.` / `pl.tensor.` 的取舍。
- [语言指南](../language/index.md) —— 这些签名所用类型背后的说明。
