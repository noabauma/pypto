...
**要带走的判断方法。** 只有当某个中间类型无法精确表示落在目标范围内的源值时，链式转换才会产生差异。请对**你自己**那条链检查这个性质，而不是把「它被展开了」当作证据。见 [LegalizeTileCast](../../dev/passes/17-legalize_tile_cast.md)。
...
- [LegalizeTileCast](../../dev/passes/17-legalize_tile_cast.md) —— cast 链的精确性。
