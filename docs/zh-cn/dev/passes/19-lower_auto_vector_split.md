...
每个拆分函数到达 [`SplitVectorKernel`](22-split_vector_kernel.md) 时都已带
...
任何 `SplitAivScopeStmt` 会到达 `ExpandMixedKernel`（pass 20）或 codegen。
...
[`ExpandMixedKernel`](20-expand_mixed_kernel.md) 跳过其单一函数级模式的转置检查——
...
- **任务并行体**（`None`）：**没有拆分轴**——两个 AIV lane 都运行**完整**区域体，由作者通过
  ...
  **原样透传区域体**（不折半、不本地化偏移、不注入 `subblock_idx`；作者的
  ...
  共同拦截。该函数仍会被标记 `split_aiv`，因此下游 [`ExpandMixedKernel`](20-expand_mixed_kernel.md) /
...
attrs，因此 [`ExpandMixedKernel`](20-expand_mixed_kernel.md) 凭该标记跳过自身的 func-mode
...
- [`ResolveBackendOpLayouts`](18-resolve_backend_op_layouts.md) —— 紧邻其前运行。
- [`ExpandMixedKernel`](20-expand_mixed_kernel.md) —— 紧邻其后运行；把
  `tile.aiv_shard` / `tile.aic_gather` 折叠为带拆分标记的 `tpush`/`tpop`。
- [`SplitVectorKernel`](22-split_vector_kernel.md) —— 下游；仅为本 pass 产生的
