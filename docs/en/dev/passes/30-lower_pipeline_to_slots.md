...
**No new IR op and no new user-facing switch.** The synthesized MemRef is shaped exactly like an author's declaration, so [`InitMemRef`](34-init_memref.md) resolves it through the same path and codegen sees no difference between a rotation the author wrote and one the compiler derived.
