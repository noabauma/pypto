...
After [`InferTileMemorySpace`](20-infer_tile_memory_space.md) has resolved `Left` / `LeftScale` / `Right` / `RightScale` and inserted any required `tile.move`s, this pass materializes the A5 scale-address bind:
...
**Pipeline position**: Immediately after [`InferTileMemorySpace`](20-infer_tile_memory_space.md), before [`ResolveBackendOpLayouts`](22-resolve_backend_op_layouts.md).
...
- Memory-space solving for MX operands: [`InferTileMemorySpace`](20-infer_tile_memory_space.md)
