...
**Pipeline position**: After [`NormalizeReturnOrder`](28-normalize_return_order.md), immediately before [`LowerPipelineToSlots`](30-lower_pipeline_to_slots.md), which in turn runs immediately before [`LowerPipelineLoops`](31-lower_pipeline_loops.md). A cross-core loop is therefore skewed (→ Sequential, no `pipeline_stages` marker) before either pass sees it, so neither slots nor replicates it.
...
- [`LowerPipelineLoops`](31-lower_pipeline_loops.md) — replicates the remaining (same-core) pipeline loops for ping-pong.
- [`CanonicalizeIOOrder`](32-canonicalize_io_order.md) — clusters same-core IO within pipeline bodies (cross-core loops no longer reach it; they are Sequential by here).
- [`SplitVectorKernel`](26-split_vector_kernel.md) — `UP_DOWN` vector split, orthogonal to the skew and composable with it.
