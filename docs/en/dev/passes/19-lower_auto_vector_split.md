...
[`SplitVectorKernel`](22-split_vector_kernel.md) already `split_aiv`-marked,
...
[`ExpandMixedKernel`](20-expand_mixed_kernel.md) to skip its single-func-mode
...
- **Task-parallel body** (`None`): there is **no split axis** — both AIV lanes run
  ...
  `split_aiv`, so downstream [`ExpandMixedKernel`](20-expand_mixed_kernel.md) /
...
[`ExpandMixedKernel`](20-expand_mixed_kernel.md) skipping its own func-mode check
...
- [`ResolveBackendOpLayouts`](18-resolve_backend_op_layouts.md) — runs
  immediately before.
- [`ExpandMixedKernel`](20-expand_mixed_kernel.md) — runs immediately after;
  folds `tile.aiv_shard` / `tile.aic_gather` into split-stamped `tpush`/`tpop`.
- [`SplitVectorKernel`](22-split_vector_kernel.md) — downstream; only stamps
