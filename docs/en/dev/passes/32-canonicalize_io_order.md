...
Cross-core `tpush`/`tpop` carry no special category — they fall through to `TileCompute` and keep their program order among siblings (cross-core software-pipelining is done upstream by [`SkewCrossCorePipeline`](29-skew_cross_core_pipeline.md); see *Cross-core (AIC↔AIV)* above).
...
- [`LowerPipelineLoops`](31-lower_pipeline_loops.md) — upstream producer of replicated regions that benefit from this pass; leaves `ForKind::Pipeline` as the scope marker this pass consumes
- [`MaterializeTensorStrides`](33-materialize_tensor_strides.md) — runs immediately after this pass (when inserted into the default pipeline); fills implicit `TensorView` strides before `InitMemRef` consumes them
- [`MemoryReuse`](36-memory_reuse.md) — runs after this pass; enforces stage buffer separation explicitly via `pipeline_membership` (this pass only shapes the schedule)
